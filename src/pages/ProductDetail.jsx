...
const ProductDetail = () => {
  ...
  const canInteract = user && (user.displayStatus === 'verified' || user.displayStatus === 'freshman')

  const handleChat = async () => {
    if (!canInteract) {
      const msg = user?.displayStatus === 'pending_offer'
        ? (language === 'zh' ? 'Offer审核中，请等待后台通过' : 'Your Offer is under review')
        : (language === 'zh' ? '请先完成认证' : 'Please verify first')
